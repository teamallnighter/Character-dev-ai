...
const EditTraits = () => {
  ...
  const { traitsId } = router.query;

  useEffect(() => {
    dispatch(fetch({ id: traitsId }));
  }, [traitsId]);
  ...
  useEffect(() => {
    if (typeof traits === 'object') {
      ...
      Object.keys(initVals).forEach((el) => (newInitialVal[el] = traits[el]));
  ...
  const handleSubmit = async (data) => {
    await dispatch(update({ id: traitsId, data }));
...
EditTraits.getLayout = function getLayout(page: ReactElement) {
  return (
