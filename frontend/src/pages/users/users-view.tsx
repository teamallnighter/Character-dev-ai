    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
          ...
          <>
            <p className={'block font-bold mb-2'}>Characters Creator</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.characters_creator &&
                      Array.isArray(users.characters_creator) &&
                      users.characters_creator.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/characters/characters-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='name'>{item.name}</td>
              ...
              {!users?.characters_creator?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
