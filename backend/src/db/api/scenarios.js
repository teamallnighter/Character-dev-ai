...
module.exports = class ScenariosDBApi {
  ...
  static async bulkImport(data, options) {
    ...
    // Bulk create items
    const scenarios = await db.scenarios.bulkCreate(scenariosData, {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    const scenarios = await db.scenarios.findOne({ where }, { transaction });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.title) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('scenarios', 'title', filter.title),
        };
      }
      ...
      if (filter.content) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('scenarios', 'content', filter.content),
        };
      }
  ...
  static async findAllAutocomplete(query, limit, offset) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('scenarios', 'title', query),
        ],
      };
    }
